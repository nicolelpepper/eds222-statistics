...
# Size as a funciton of lat
ggplot(data = pie_crab, aes(x = latitude, y = size)) + 
  geom_point() +
  labs(x="Latitude", y="Crab Body Size", title = "Crab Body Size as a Function of Latitude") +
  theme(aspect.ratio = 1)


#Water function as a lat
ggplot(data = pie_crab, aes(x = latitude, y = water_temp)) + 
  geom_point() +
  labs(x="Latitude", y="Water Temp", title = "Water Temp as a Function of Latitude")
...
(i) the size of crab and latitude: .55
(ii) water temperature and latitude: -.8
...
```{r}
print("The correlation of latitude and crab size is:")
cor(pie_crab$latitude, pie_crab$size)


print("The correlation of latitude and water temp is:")
...
**Exercise:**
## Started losing me at this point
# Process of interpreting this
...
When using ordinary leaf squares we have to be careful in how we interpret this because it assumes that you have normally distributed data - we can't have a negative size

```{r}
# Questions 1:
# lm([target] ~ [predictor], data = [data source])

# for size: target = size   ~   predictor = latitude
lm(size ~ latitude, data = pie_crab)

            #Coefficients:
            #(Intercept)     latitude  
              #  -3.6244       0.4851  
print("Summary for lat x size:")
summary(lm(size ~ latitude, data = pie_crab))


# for water temp: target = size   ~   predictor = latitude
...
Slope is 0.4815 (rate of change)
Intercept is -3.62 (this is not easily interpretable because size can't be negative)
###Assumes normally distributed data... but you can still use it on non-normal data?? 
...
pie_crab2 <- pie_crab %>%
  mutate(predicted_size = predict(crab_lm),
        residual_size = residuals(crab_lm))
...
```{r}
ggplot(pie_crab2, aes( x = latitude, y = residual_size)) +
  geom_point() +
