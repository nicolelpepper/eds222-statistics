...
```{r}
#Q1
c_size <- summary(lm(size ~ latitude, data = pie_crab))

summary(lm(water_temp ~ latitude, data = pie_crab))

```

#Q2 The regression model on the water temperature did better - it has a significantly higher R2 (0.916) then the R2 for the latitude (0.348).
...
# Sum squared errors  
# Resid() is getting the errors of the dataset()
# ^2 we square the errors
# Sum () taking the sum of everything in ()
sse <- sum(resid(c_size)^2)


# SST

# Get the mean of the y values
y_bar <- mean(pie_crab$size)


# Select crab size iterator - mean y
# ^2 square it
# Sum () it all
sst <- sum((pie_crab$size - y_bar)^2)

# To get the r^2 we divide sse/sst and subtract it from 1
rsquare <- 1 - (sse/sst)
...
# Check the type of the mpg column
class(mpg$year)

mpg_new <- mpg %>%
  mutate(year_fct = factor(mpg$year))

class(mpg_new$year_fct)
```
...
ggplot(data = mpg_new, aes(x = year_fct, y = hwy)) +
  geom_boxplot()


ggplot(data = mpg_new, aes(x = hwy, color = year_fct)) + 
...
```{r}
# Q1
summary(lm(hwy ~ year_fct, data = mpg_new))


```

Q2. Intercept is 23.4274, intercept tell you mean the first entry, value of the reference level () Q3. The Slope is 0.0256, barely increasing - fuel efficiency isn't getting better over time. Coefficients Means the difference relative to the reference level (between the two distributions), which in this case is minimal.

B0 = mean + Difference relative to the two years. Coefficients
