...
4.  Describe what each following R function returns.
    ...
    5.  `rbinom(): generates random binomial values`
        ...
        `NOTE` rs (rnorm or rbindom) are random number generator)
...
obs <- c(0, 0, 1, 1)
dbinom(obs, size = 1, prob = 0.6)
# sequence returns the probability of each outcome (assumption is the prob of 1 so its whatevers is remaining for 0 or prob of 1 if 1)
...
To get the prob. of the sequence you multiply the individual probs: (because of biferkation)
...
1.  Fill in the blanks below to specify a logistic regression in statistical notation.
    ...
    -   What links the response parameter to the linear predictor

    -   You need to link it with a transformation because it doesn't fit the requirements of traditional OLS

    -   LINK IS A TRANSFORMATION - moves to satisfy needs of OLS (to find the relationship between two variables)

    -   The transformation is illustrated well by the y binomial options and the s shape to fit to probability

    Only for bernoulis

    P can be any number between 0 & 1, positive\
    $$
    \begin{align}
    y &\sim Bernoulli (p) \\
    logit(p) &= B0 + B1 x
...
-   The logit is continuous and symmetrical

-   the y axis is the part we are modeling to figure out p

-   We cant

1.  The *logistic* function is the inverse of the logit function. Without using R or a calculator, what's the value of $logistic(logit(0))$? How about $logistic(logit(1000))$? `<- went through these very fast, I got lost.`

    1.  What kinds of numbers can the logit function operate on? Conversely, what inputs would yield an undefined result? Why?

        `logit(p)` Taking a number \[0,1\] and returns between \[-inf, +inf\]

        `logistic(x)`\[-inf, +inf\] and returns \[0,1\] aka the inverse of the logit (undoes or "cancels" it). We have the use the logistic on the logit of p - to undo it and get the p back
        ...
        *This is undefined because there is no power we can raise a number to get it*

        we have the logit of p - use logistic on logit to undo it and get the p back

        1.  What about the logistic?

            ### Workflow Explanation:

            Call GLM

            1\) Defines a likelihood function: how likely are these coefficients given the data. We know the data how likely is any given set of coefficients. Choose fam based on distribution of the data

            2\) Explore the potential coefficients

            1.  ex. this is the DEM Steps to show likelihood to find the most likely coefficients

            2.  Results are a max likelihood matrix plus some uncertainty

                ### Example:

                Scenario: elevation gradient for where we find white bark pines (high elevation species). Researchers took survey of presence (yes or no) across elevation gradient.

                Question: How does elevation impact white bark presence

                Data: yes or no presence

                We need the prob line for this baseline before we can start asking more questions and integrating other variables.

                Option to integrate other variables predictions (like temperature)

            Gather data:

            -   Formulate hypothesis

            -   Rearrange data to test hypothesis

            -   P value = how likeli is this to happen

2.  Imagine a "logistic" regression without a link function. I.e., $p$ is linearly related to $x$. Give an example of values for $\beta_0,\beta_1,x$ that would break your regression model. What constraint on $p$ did you break?

    `logit(p) = B0 + B1 x`

3.  In your own words, describe why the logit link function is necessary for logistic regression to work.
...
4.  Use ggplot to create a logistic curve. Your x-axis should cover the range -5 to 5. Use $\beta_0=0,\beta_1=1$ as your coefficients.\
...
penguins <- drop_na(penguins, is_male, bill_length_mm)

ggplot(penguins, aes(bill_length_mm, is_male)) + 
  geom_point(shape = 21)
glm(is_male ~ bill_length_mm,
